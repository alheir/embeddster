...
class GodModeWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.station_table.setHorizontalHeaderLabels(
            ["Station", "Roll", "Roll Δt", "Pitch", "Pitch Δt", "Yaw", "Yaw Δt"]
        )
        ...
        select_all_btn.clicked.connect(
            lambda: [cb.setChecked(True) for cb in self.angle_group_checks]
        )
        ...
        select_none_btn.clicked.connect(
            lambda: [cb.setChecked(False) for cb in self.angle_group_checks]
        )
        ...
        select_all_btn.setMaximumWidth(40)
        select_all_btn.clicked.connect(
            lambda: [cb.setChecked(True) for cb in self.random_group_checks]
        )
        ...
        select_none_btn.setMaximumWidth(40)
        select_none_btn.clicked.connect(
            lambda: [cb.setChecked(False) for cb in self.random_group_checks]
        )
        ...
        self.rx_table.setHorizontalHeaderLabels(
            ["Timestamp", "CAN ID", "DLC", "Type", "Data (Hex)", "Data (ASCII/Bin)"]
        )
        ...
        if filename:
            try:
                ...
                self.log_text.append(
                    f"✅ Exported {len(self.message_history)} messages to {filename}"
                )
            except Exception as e:
            ...
            for idx, angle in enumerate(["R", "C", "O"]):
                ...
                if data[angle]["value"] is not None:
                    self.station_table.setItem(
                        station, value_col, QTableWidgetItem(f"{data[angle]['value']}°")
                    )

                    elapsed = time.time() - data[angle]["time"]
                    time_str = self.format_elapsed_time(elapsed)
                    self.station_table.setItem(station, time_col, QTableWidgetItem(time_str))
                else:
    ...
    def send_manual_message(self):
        try:
            ...
            self.log_text.append(
                f"✅ Manual: ID=0x{can_id:03X}, Data={data.hex()} ({len(data)} bytes)"
            )
        except Exception as e:
