...
from src.package.Station import group_label
...
class PlotWidget(QWidget):
    def __init__(self, station_index, angle_histories):
        ...
        title = group_label(station_index)
    ...
    def updatePlot(self, angle_histories=None):
        if angle_histories:
            self.angle_histories = angle_histories
        for history, curve in zip(self.angle_histories, self.curves):
            if history:
                times, values = zip(*history)
                relative_times = np.array(times) - self.start_time
